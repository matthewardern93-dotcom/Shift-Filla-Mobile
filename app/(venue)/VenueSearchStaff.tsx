...
import { usePreviouslyHiredStaff } from '../../hooks/usePreviouslyHiredStaff'; // Import the new hook

const VenueSearchStaff = () => {
  const router = useRouter();
  const { staff, isLoading, error } = usePreviouslyHiredStaff(); // Use the hook

  const renderLeftActions = (worker, close) => {
    return (
      <TouchableOpacity
  ...
  const WorkerCard = ({ worker }) => {
    let swipeableRef = React.useRef(null);
    ...
    return (
        <Swipeable ref={swipeableRef} renderLeftActions={() => renderLeftActions(worker, closeSwipeable)} friction={2}>
            <Pressable onPress={() => handleCardPress(worker)}>
                <View style={styles.card}>
                    ...
                    <View style={styles.cardContent}>
                        ...
                        <View style={styles.workerStats}>
                            <Text style={styles.rating}>⭐ {worker.rating?.average.toFixed(1) || 'N/A'}</Text>
  ...
  const renderContent = () => {
    ...
    if (staff.length === 0) {
        return <View style={styles.centered}><Text style={styles.emptyText}>You haven't hired any staff yet. Completed shifts will appear here.</Text></View>;
    }
