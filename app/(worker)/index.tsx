...
import { View, Text, StyleSheet, TouchableOpacity, Modal, Alert, ScrollView, ActivityIndicator, FlatList } from 'react-native';
import { useFocusEffect, useRouter } from 'expo-router';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { format, parseISO } from 'date-fns';
import { Filter, X, CheckSquare, Square } from 'lucide-react-native';

import WorkerScreenTemplate from '../../components/templates/WorkerScreenTemplate';
import { Colors } from '../../constants/colors';
import { Shift, Job, WorkerProfile } from '../../types';
...
const WorkerHome = () => {
    ...
    const [availability, setAvailability] = useState<Record<string, { customStyles?: { container?: { backgroundColor?: string } } }>>({});

    const { jobs, hasNewJobs, isLoading: jobsLoading, subscribeToJobs, markJobsAsViewed, cleanup: cleanupJobs } = useJobStore();
    const { shifts: availableShifts, isLoading: shiftsLoading, fetchAvailableShifts, cleanup: cleanupShifts } = useAvailableShiftsStore();

    const workerName = (profile && profile.userType === 'worker') ? (profile as WorkerProfile).firstName : '';
    const userSkills = (profile && profile.userType === 'worker' && Array.isArray((profile as WorkerProfile).skills)) ? (profile as WorkerProfile).skills : [];
    ...
    }, [user?.uid, subscribeToJobs, fetchAvailableShifts, cleanupJobs, cleanupShifts]);
    ...
    const fetchAppliedShifts = async () => {
        try {
            ...
            setAppliedShifts(storedShifts.map(s => s[1] ? JSON.parse(s[1]).id : '').filter(id => id));
        } catch (e) {
    ...
    useFocusEffect(useCallback(() => { fetchAppliedShifts(); fetchAvailability(); }, []));
    ...
    const filteredAndSortedShifts = useMemo(() => {
        return (availableShifts as Shift[])
            .filter(s => {
                ...
                if (availability[dateString]?.customStyles?.container?.backgroundColor === Colors.danger) return false;
                if (selectedRoles.length > 0 && !selectedRoles.includes(s.role)) return false;
                return true;
            })
            .sort((a, b) => {
                if (a.status === 'offered_to_worker' && b.status !== 'offered_to_worker') return -1;
                if (b.status === 'offered_to_worker' && a.status !== 'offered_to_worker') return 1;
                return toDateSafe(a.startTime).getTime() - toDateSafe(b.startTime).getTime();
            });
    ...
    const handleShiftPress = (shift: Shift) => {
        const route = shift.status === 'offered_to_worker' ? '/(worker)/WorkerViewShiftOfferCard' : '/(worker)/WorkerViewShiftDetailsCard';
    ...
    const renderContent = () => {
        ...
        if (activeTab === 'Shifts') {
            ...
            return filteredJobs.length > 0 ? (
                filteredJobs.map(job => <WorkerJobCard key={job.id} item={job} onPress={() => handleJobPress(job)} />)
        ...
        <WorkerScreenTemplate>
            <View style={styles.flexView}>
                ...
                <View style={styles.tabContainer}>
                    {['Shifts', 'Jobs'].map(tab => (
                        <TouchableOpacity key={tab} style={[styles.tab, activeTab === tab && styles.activeTab]} onPress={() => handleTabPress(tab)}>
                            <View style={styles.tabItemContainer}>
                                <Text style={[styles.tabText, activeTab === tab && styles.activeTabText]}>{tab}</Text>
                                {tab === 'Jobs' && hasNewJobs && <View style={styles.newJobDot} />}
                            </View>
                        </TouchableOpacity>
                    ))}
                </View>
                ...
                <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setModalVisible(false)}>
                    <TouchableOpacity activeOpacity={1} style={styles.modalContent} onPress={e => e.stopPropagation()}>
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }) => (
                                <TouchableOpacity style={styles.option} onPress={() => toggleRoleSelection(item)}>
                                    {tempSelectedRoles.includes(item) ? <CheckSquare size={24} color={Colors.primary} /> : <Square size={24} color={Colors.textSecondary} />}
                                    <Text style={styles.optionText}>{item}</Text>
                                </TouchableOpacity>
                            )}
                            ListEmptyComponent={<Text style={styles.noDataText}>You have no skills on your profile.</Text>}
                        />
...
const styles = StyleSheet.create({
    flexView: { flex: 1 },
    loader: { marginTop: 50 },
    noDataText: { textAlign: 'center', marginTop: 20, fontSize: 16, color: Colors.textSecondary },
    ...
    tabItemContainer: { flexDirection: 'row', alignItems: 'center', position: 'relative' },
    newJobDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: Colors.primary, position: 'absolute', right: -12, top: -2 },
    ...
    clearButtonText: { fontSize: 16, fontWeight: 'bold', color: Colors.textSecondary },
