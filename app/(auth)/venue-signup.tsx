...
import { ChevronDown, ArrowLeft, Image as ImageIcon, Upload, UploadCloud, Check } from 'lucide-react-native';
import TextInput from '../../components/input';

const formSchema = z.object({
  venueName: z.string().min(2, "Business name is required."),
...
const posSystems: PickerItem[] = ["Square", "Lightspeed", "Toast", "Clover", "Other"].map(s => ({label: s, value: s}));
...
export default function VenueSignUpScreen() {
  ...
  const onSubmit = async (values: FormData) => {
    ...
    try {
      ...
      let errorMessage = (error as Error).message || 'An unexpected error occurred.';
  ...
  return (
    <ScrollView style={styles.container}>
        ...
        <Controller name="venueName" control={control} render={({ field }) => <TextInput style={styles.input} placeholder="Business Name" onBlur={field.onBlur} onChangeText={field.onChange} value={field.value} placeholderTextColor={Colors.textSecondary} />} />
        {errors.venueName && <Text style={styles.errorText}>{errors.venueName.message}</Text>}
        
        <Controller name="companyNumber" control={control} render={({ field }) => <TextInput style={styles.input} placeholder="Company Number (NZBN)" onBlur={field.onBlur} onChangeText={field.onChange} value={field.value} placeholderTextColor={Colors.textSecondary} />} />
        
        <Controller name="address" control={control} render={({ field }) => <TextInput style={styles.input} placeholder="Business Address" onBlur={field.onBlur} onChangeText={field.onChange} value={field.value} placeholderTextColor={Colors.textSecondary} />} />
        ...
        <Controller name="contactName" control={control} render={({ field }) => <TextInput style={styles.input} placeholder="Contact Person Name" onBlur={field.onBlur} onChangeText={field.onChange} value={field.value} placeholderTextColor={Colors.textSecondary} />} />
        {errors.contactName && <Text style={styles.errorText}>{errors.contactName.message}</Text>}

        <Controller name="phone" control={control} render={({ field }) => <TextInput style={styles.input} placeholder="Contact Phone" onBlur={field.onBlur} onChangeText={field.onChange} value={field.value} keyboardType="phone-pad" placeholderTextColor={Colors.textSecondary} />} />
        {errors.phone && <Text style={styles.errorText}>{errors.phone.message}</Text>}

        <Controller name="email" control={control} render={({ field }) => <TextInput style={styles.input} placeholder="Email" onBlur={field.onBlur} onChangeText={field.onChange} value={field.value} keyboardType="email-address" autoCapitalize="none" placeholderTextColor={Colors.textSecondary} />} />
        {errors.email && <Text style={styles.errorText}>{errors.email.message}</Text>}

        <Controller name="password" control={control} render={({ field }) => <TextInput style={styles.input} placeholder="Password (min. 8 characters)" onBlur={field.onBlur} onChangeText={field.onChange} value={field.value} secureTextEntry placeholderTextColor={Colors.textSecondary} />} />
        {errors.password && <Text style={styles.errorText}>{errors.password.message}</Text>}

        <Text style={styles.sectionHeader}>About Your Venue</Text>
        <Controller name="about" control={control} render={({ field }) => <TextInput style={[styles.input, styles.textArea]} placeholder="Tell workers about your venue..." multiline onBlur={field.onBlur} onChangeText={field.onChange} value={field.value} placeholderTextColor={Colors.textSecondary} />} />
        ...
        <View style={styles.checkboxContainer}>
            <TouchableOpacity onPress={() => setValue('agreeToTerms', !agreeToTerms, { shouldValidate: true })} style={[styles.checkbox, agreeToTerms && styles.checkboxChecked]}>
                {agreeToTerms && <Check size={18} color={Colors.white} />}
            </TouchableOpacity>
